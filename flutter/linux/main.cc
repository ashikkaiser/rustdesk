...
#define CLOUDYDESK_LIB_PATH "libcloudydesk.so"
typedef bool (*CloudyDeskCoreMain)();
bool gIsConnectionManager = false;

void print_help_install_pkg(const char* so);

bool flutter_cloudydesk_core_main() {
   void* libcloudydesk = dlopen(CLOUDYDESK_LIB_PATH, RTLD_LAZY);
   if (!libcloudydesk) {
      fprintf(stderr,"Failed to load \"libcloudydesk.so\"\n");
      char* error;
   ...
   auto core_main = (CloudyDeskCoreMain) dlsym(libcloudydesk,"cloudydesk_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"cloudydesk_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}
...
int main(int argc, char** argv) {
  if (!flutter_cloudydesk_core_main()) {
      return 0;
  }
