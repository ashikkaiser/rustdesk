...
#
# Script to build F-Droid release of CloudyDesk
#
# Copyright (C) 2024, The CloudyDesk Authors
...
#
# - versionName from https://github.com/cloudydesk/cloudydesk/releases/download/fdroid-version/cloudydesk-version.txt
# - versionCode from https://github.com/cloudydesk/cloudydesk/releases/download/fdroid-version/cloudydesk-version.txt
...
arm64-v8a)
	...
	CLOUDYDESK_FEATURES='flutter,hwcodec'
	;;
armeabi-v7a)
...
prebuild)
	...
	# Flutter used to compile main Cloudydesk library
	...
	# Install Flutter version for CloudyDesk library build
...
build)
	...
	cargo ndk \
		...
		build \
		--release \
		--features "${CLOUDYDESK_FEATURES}"

	mkdir -p "flutter/android/app/src/main/jniLibs/${ANDROID_ABI}"

	cp "target/${RUST_TARGET}/release/liblibcloudydesk.so" \
		"flutter/android/app/src/main/jniLibs/${ANDROID_ABI}/libcloudydesk.so"
