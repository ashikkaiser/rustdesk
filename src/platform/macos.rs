...
const UPDATE_TEMP_DIR: &str = "/tmp/.cloudydeskupdate";
...
// remove just one app from all the permissions: tccutil reset All com.carriez.cloudydesk
...
fn correct_app_name(s: &str) -> String {
    ...
    if let Some(bundleid) = get_bundle_id() {
        s = s.replace("com.carriez.cloudydesk", &bundleid);
    }
    s = s.replace("cloudydesk", &crate::get_app_name().to_lowercase());
    s = s.replace("CloudyDesk", &crate::get_app_name());
    s
}
...
pub fn start_os_service() {
    ...
    /* // mouse/keyboard works in prelogin now with launchctl asuser.
       ...
       // one way is to start with `launchctl asuser <uid> open -n -a /Applications/CloudyDesk.app/ --args --cm`,
       // this way --cm is started with the user privilege, but we will have problem to start another CloudyDesk.app
...
pub fn update_me() -> ResultType<()> {
    ...
    let cmd = std::env::current_exe()?;
    // CloudyDesk.app/Contents/MacOS/CloudyDesk
    let app_dir = cmd
        .parent()
        .and_then(|p| p.parent())
        .and_then(|p| p.parent())
    ...
    if is_installed_daemon && !is_service_stopped {
        ...
        let update_body = format!(
            r#"
do shell script "
pgrep -x 'CloudyDesk' | grep -v {} | xargs kill -9 && rm -rf /Applications/CloudyDesk.app && ditto '{}' /Applications/CloudyDesk.app && chown -R {}:staff /Applications/CloudyDesk.app && xattr -r -d com.apple.quarantine /Applications/CloudyDesk.app
" with prompt "CloudyDesk wants to update itself" with administrator privileges
    "#,
            std::process::id(),
            app_dir,
            get_active_username()
...
fn extract_dmg(dmg_path: &str, target_dir: &str) -> ResultType<()> {
    let mount_point = "/Volumes/CloudyDeskUpdate";
...
fn update_extracted(target_dir: &str) -> ResultType<()> {
    let exe_path = format!("{}/CloudyDesk.app/Contents/MacOS/CloudyDesk", target_dir);
    let _child = unsafe {
