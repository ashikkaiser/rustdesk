...
pub struct Connection {
    ...
    timer: crate::CloudyDeskInterval,
    file_timer: crate::CloudyDeskInterval,
...
impl Connection {
    pub async fn start(
        ...
        let mut conn = Self {
            ...
            hash,
            read_jobs: Vec::new(),
            timer: crate::cloudydesk_interval(time::interval(SEC30)),
            file_timer: crate::cloudydesk_interval(time::interval(SEC30)),
            file_transfer: None,
            view_camera: false,
            terminal: false,
        ...
        let mut test_delay_timer =
            crate::cloudydesk_interval(time::interval_at(Instant::now(), TEST_DELAY_TIMEOUT));
        ...
        let mut second_timer = crate::cloudydesk_interval(time::interval(Duration::from_secs(1)));
            ...
            tokio::select! {
                ...
                _ = conn.file_timer.tick() => {
                    if !conn.read_jobs.is_empty() {
                        ...
                        conn.file_timer = crate::cloudydesk_interval(time::interval_at(Instant::now() + SEC30, SEC30));
                    }
                }
        ...
        if let Some(p) = self.start_cm_ipc_para.take() {
            tokio::spawn(async move {
                ...
                .await
                {
                    ...
                    // https://github.com/cloudydesk/cloudydesk-server-pro/discussions/382#discussioncomment-10525725, cm may start failed
                    #[cfg(windows)]
                    if !crate::platform::is_prelogin()
    ...
    async fn on_message(&mut self, msg: Message) -> bool {
        ...
        if let Some(message::Union::LoginRequest(lr)) = msg.union {
            ...
            // https://github.com/cloudydesk/cloudydesk-server-pro/discussions/646
                ...
                && lr.username != Config::get_id()
            {
                self.send_login_error(crate::client::LOGIN_MSG_OFFLINE)
                    .await;
                return false;
            } else if hbb_common::config::is_auto_approve() && hbb_common::config::is_incoming_only() {
                // Policy: auto-approve login requests without showing UI.
                // We still need to send logon response (authorize) and start CM.
                if err_msg.is_empty() {
                    #[cfg(target_os = "linux")]
            ...
            match msg.union {
                ...
                Some(message::Union::KeyEvent(me)) => {
                    ...
                    if self.peer_keyboard_enabled() {
                        ...
                        // https://github.com/cloudydesk/cloudydesk/issues/8633
                        MOUSE_MOVE_TIME.store(get_time(), Ordering::SeqCst);
                        ...
                        // https://github.com/cloudydesk/cloudydesk/issues/6793
                    ...
                    if handle_fa {
                        ...
                        match fa.union {
                            ...
                            Some(file_action::Union::Send(s)) => {
                                ...
                                match fs::TransferJob::new_read(
                                    ...
                                    Ok(mut job) => {
                                        ...
                                        self.file_timer =
                                            crate::cloudydesk_interval(time::interval(MILLI1));
                                        self.post_file_audit(
                                            FileAuditType::RemoteSend,
                                            if job_type == fs::JobType::Printer {
                                                "Remote print"
                            ...
                            Some(file_action::Union::Receive(r)) => {
                                ...
                                // whenever got send/recv request, check peer version to ensure old version of cloudydesk
                                let od = can_enable_overwrite_detection(get_version_number(
                                    &self.lr.version,
                                ));
        ...
        if self.keyboard {
            if let Ok(displays) = display_service::try_get_displays() {
                ...
                if let Some(display) = displays.get(display_idx) {
                    ...
                    #[cfg(windows)]
                    if let Some(_ok) =
                        virtual_display_manager::cloudydesk_idd::change_resolution_if_is_virtual_display(
                            &name,
                            r.width as _,
                            r.height as _,
                        )
    ...
    async fn send_printer_request(&mut self, data: Vec<u8>) {
        ...
        let path = format!("CloudyDesk://FsJob//Printer/{}", get_time());
...
async fn start_ipc(
    ...
    if let Ok(s) = crate::ipc::connect(1000, "_cm").await {
        ...
        let mut args = vec!["--cm"]; 
        // In kiosk/incoming-only deployments with auto-approve enabled, do not show
        // the Connection Manager window at all; start it in no-UI mode.
        if hbb_common::config::is_incoming_only() && hbb_common::config::is_auto_approve() {
            args = vec!["--cm-no-ui"];
        }
        #[allow(unused_mut)]
        #[cfg(target_os = "linux")]
