...
fn get_plugin_source_list() -> Vec<PluginSource> {
    ...
    // vec![PluginSource {
    //     name: "cloudydesk".to_string(),
    //     url: "https://raw.githubusercontent.com/fufesou/cloudydesk-plugins/main".to_string(),
...
fn elevate_install(
    ...
    crate::platform::elevate(args, "CloudyDesk wants to install then plugin")
}
...
fn elevate_uninstall(plugin_id: &str) -> ResultType<bool> {
    crate::platform::elevate(
        ...
        "CloudyDesk wants to uninstall the plugin",
    )
}
