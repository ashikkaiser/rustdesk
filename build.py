...
hbb_name = 'cloudydesk' + ('.exe' if windows else '')
...
def generate_control_file(version):
    ...
    content = """Package: cloudydesk
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: cloudydesk <info@cloudydesk.com>
Homepage: https://cloudydesk.com
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/cloudydesk')
    system2('mkdir -p tmpdeb/etc/cloudydesk/')
    system2('mkdir -p tmpdeb/etc/pam.d/')
    system2('mkdir -p tmpdeb/usr/share/cloudydesk/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/cloudydesk || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/share/cloudydesk/')
    system2(
        'cp ../res/cloudydesk.service tmpdeb/usr/share/cloudydesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/cloudydesk.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/cloudydesk.svg')
    system2(
        'cp ../res/cloudydesk.desktop tmpdeb/usr/share/applications/cloudydesk.desktop')
    system2(
        'cp ../res/cloudydesk-link.desktop tmpdeb/usr/share/applications/cloudydesk-link.desktop')
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/cloudydesk/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/cloudydesk/')
    system2(
        'cp ../res/pam.d/cloudydesk.debian tmpdeb/etc/pam.d/cloudydesk')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/cloudydesk/files/polkit && chmod a+x tmpdeb/usr/share/cloudydesk/files/polkit")

    system2('mkdir -p tmpdeb/DEBIAN')
    generate_control_file(version)
    system2('cp -a ../res/DEBIAN/* tmpdeb/DEBIAN/')
    md5_file_folder("tmpdeb/")
    system2('dpkg-deb -b tmpdeb cloudydesk.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('cloudydesk.deb', '../cloudydesk-%s.deb' % version)
    os.chdir("..")
...
def build_deb_from_folder(version, binary_folder):
    ...
    system2('rm tmpdeb/usr/bin/cloudydesk || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/share/cloudydesk/')
    system2(
        'cp ../res/cloudydesk.service tmpdeb/usr/share/cloudydesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/cloudydesk.png')
...
def build_flutter_dmg(version, features):
    ...
    # copy dylib
    system2(
        "cp target/release/liblibcloudydesk.dylib target/release/libcloudydesk.dylib")
    os.chdir('flutter')
    system2('flutter build macos --release')
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/CloudyDesk.app/Contents/MacOS/')
    '''
    system2(
        "create-dmg --volname \"CloudyDesk Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon CloudyDesk.app 200 190 --hide-extension CloudyDesk.app cloudydesk.dmg ./build/macos/Build/Products/Release/CloudyDesk.app")
    os.rename("cloudydesk.dmg", f"../cloudydesk-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_arch_manjaro(version, features):
    ...
    os.chdir('flutter')
    system2('flutter build linux --release')
    system2(f'strip {flutter_build_dir}/lib/libcloudydesk.so')
    os.chdir('../res')
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/cloudydesk.exe')
    os.chdir('../..')
    if os.path.exists('./cloudydesk_portable.exe'):
        os.replace('./target/release/cloudydesk-portable-packer.exe',
                   './cloudydesk_portable.exe')
    else:
        os.rename('./target/release/cloudydesk-portable-packer.exe',
                  './cloudydesk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/cloudydesk_portable.exe')
    os.rename('./cloudydesk_portable.exe', f'./cloudydesk-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/cloudydesk-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/cloudydesk.exe')
        system2('mv target/release/cloudydesk.exe target/release/CloudyDesk.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\cloudydesk.exe')
        else:
            print('Not signed')
        system2(
            f'cp -rf target/release/CloudyDesk.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/cloudydesk-{version}-win7-install.exe')
        system2('mv ../../{res_dir}/cloudydesk-{version}-win7-install.exe ../..')
        ...
        if flutter:
            ...
            system2('cargo build --release --features ' + features)
            system2('git checkout src/ui/common.tis')
            system2('strip target/release/cloudydesk')
            system2('ln -s res/pacman_install && ln -s res/PKGBUILD')
            system2('HBB=`pwd` makepkg -f')
        system2('mv cloudydesk-%s-0-x86_64.pkg.tar.zst cloudydesk-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        # pacman -U ./cloudydesk.pkg.tar.zst
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/cloudydesk-%s-0.x86_64.rpm ./cloudydesk-%s-fedora28-centos8.rpm' % (
                version, version))
        # yum localinstall cloudydesk.rpm
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/cloudydesk-%s-0.x86_64.rpm ./cloudydesk-%s-suse.rpm' % (
                version, version))
        # yum localinstall cloudydesk.rpm
    else:
        if flutter:
            ...
            if osx:
                system2(
                    'strip target/release/bundle/osx/CloudyDesk.app/Contents/MacOS/cloudydesk')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/CloudyDesk.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/cloudydesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/CloudyDesk.app/Contents/MacOS/cloudydesk
    #rcodesign sign --p12-file ~/.p12/cloudydesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/CloudyDesk.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/cloudydesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/CloudyDesk.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/CloudyDesk.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/CloudyDesk.app
    '''.format(pa))
                system2(
                    'create-dmg "CloudyDesk %s.dmg" "target/release/bundle/osx/CloudyDesk.app"' % version)
                os.rename('CloudyDesk %s.dmg' %
                          version, 'cloudydesk-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    #rcodesign sign --p12-file ~/.p12/cloudydesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./cloudydesk-{1}.dmg
    codesign -s "Developer ID Application: {0}" --force --options runtime ./cloudydesk-{1}.dmg
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple cloudydesk-{1}.dmg
    # verify:  spctl -a -t exec -v /Applications/CloudyDesk.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
                # build deb package
                system2(
                    'mv target/release/bundle/deb/cloudydesk*.deb ./cloudydesk.deb')
                system2('dpkg-deb -R cloudydesk.deb tmpdeb')
                system2('mkdir -p tmpdeb/usr/share/cloudydesk/files/systemd/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                ...
                system2(
                    'cp res/cloudydesk-link.desktop tmpdeb/usr/share/applications/cloudydesk-link.desktop')
                os.system('mkdir -p tmpdeb/etc/cloudydesk/')
                os.system('cp -a res/startwm.sh tmpdeb/etc/cloudydesk/')
                os.system('mkdir -p tmpdeb/etc/X11/cloudydesk/')
                os.system('cp res/xorg.conf tmpdeb/etc/X11/cloudydesk/')
                os.system('cp -a DEBIAN/* tmpdeb/DEBIAN/')
                os.system('mkdir -p tmpdeb/etc/pam.d/')
                os.system('cp pam.d/cloudydesk.debian tmpdeb/etc/pam.d/cloudydesk')
                system2('strip tmpdeb/usr/bin/cloudydesk')
                system2('mkdir -p tmpdeb/usr/share/cloudydesk')
                system2('mv tmpdeb/usr/bin/cloudydesk tmpdeb/usr/share/cloudydesk/')
                system2('cp libsciter-gtk.so tmpdeb/usr/share/cloudydesk/')
                md5_file_folder("tmpdeb/")
                system2('dpkg-deb -b tmpdeb cloudydesk.deb; /bin/rm -rf tmpdeb/')
                os.rename('cloudydesk.deb', 'cloudydesk-%s.deb' % version)
